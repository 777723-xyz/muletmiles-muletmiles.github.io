...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_LMBS');
	...
	Moghunter.lmbsKeyMenu = String(Moghunter.parameters['Menu Key']);
    ...
    Moghunter.lmbsKeySkillBackup = String('d');
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === "lmbs_set_turn_duration")  {
		var newDuration	= Number(args[1]);
		BattleManager._lmbsTurn = [0,newDuration];
	};	
	return true;
};
...
BattleManager.lmbsStart = function() {	
    ...
    $gameParty.battleMembers().forEach(function(battler) {
		...
		battler._lmbs_X = ($gameSystem._lmbsScreenSize[0] + 500) + (80 * index);
    ...
    $gameTroop.members().forEach(function(battler) {
        ...
        battler.lmbsSetTarget();
		battler.lmbsMakeSkillList();
		battler._lmbs_X = ($gameSystem._lmbsScreenSize[1] - 580) - (80 * index);
...
BattleManager.lmbsUpdateCommand = function() {
	...
	if (Input.isTriggered(Moghunter.lmbsKeySkill) || Input.isTriggered(Moghunter.lmbsKeySkillBackup)) {this._lmbsActor.lmbsSkillActionActor()};
    if (Input.isTriggered('up')) {
