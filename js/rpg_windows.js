...
Window_Base.prototype.obtainEscapeParam = function(textState) {
    ...
    if (arr) {
        ...
        var arr = /^\[\c+\]/.exec(textState.text.slice(textState.index));
        if(arr){
            textState.index += arr[0].length;
            return String(arr[0].slice(1));
        }
        return '';
    }
};
...
Window_Base.prototype.processEscapeCharacter = function(code, textState) {
    ...
    case 'S':
        AudioManager.stopSe();
        var test = {"name":"Attack1", "volume":100, "pitch":100, "pan":0};
        //test.name = "Attack1";
        test.name = String("Tales of Niceria (" + this.obtainEscapeParam(textState) + ")");
